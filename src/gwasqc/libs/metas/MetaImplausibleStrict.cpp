...
MetaImplausibleStrict::MetaImplausibleStrict(unsigned int heap_size) throw (MetaException): MetaNumeric(heap_size), n_less(0), n_greater(0) {
	...
	if (numeric) {
		...
		if (n > current_heap_size) {
			...
			if (new_data == NULL) {
				free(data);
				data = NULL;
				throw MetaException("MetaImplausibleStrict", "put( char* )", __LINE__, 3, current_heap_size * sizeof(double));
			}
			data = new_data;
		}
...
void MetaImplausibleStrict::print_html(ostream& stream, char path_separator) {
	if (!numeric) {
		...
		stream << "<tr><td>&lt; " << less << "</td><td>" << n_less << "</td></tr>";
		stream << "<tr><td>&gt; " << greater << "</td><td>" << n_greater << "</td></tr>";
		stream << "</table>";
		stream << "</div>";
