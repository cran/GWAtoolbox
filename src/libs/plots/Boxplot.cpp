...
Boxplot* Boxplot::create(const char* name, double* sorted_data, int data_size, double median) throw (PlotException) {
	...
	if (name == NULL) {
		throw PlotException(1, "Boxplot", "create( ... )", "name", "NULL");
	}

	if (strlen(name) <= 0) {
		throw PlotException(2, "Boxplot", "create( ... )", "name", (const char*)NULL);
	}

	if (sorted_data == NULL) {
		throw PlotException(1, "Boxplot", "create( ... )", "sorted_data", "NULL");
	}

	if (data_size <= 0) {
		throw PlotException(3, "Boxplot", "create( ... )", "data_size", data_size);
	}
	...
	if ((boxplot->tukey = (double*)malloc(TUKEY_SIZE * sizeof(double))) == NULL) {
		throw PlotException(0, "Boxplot", "create( ... )", "tukey", TUKEY_SIZE * sizeof(double));
	}

	if ((boxplot->observations = (int*)malloc(sizeof(int))) == NULL) {
		throw PlotException(0, "Boxplot", "create( ... )", "observations", sizeof(int));
	}

	if ((boxplot->names = (char**)malloc(sizeof(char*))) == NULL) {
		throw PlotException(0, "Boxplot", "create( ... )", "names", sizeof(char*));
	}
	...
	if (boxplot->outliers_cnt > 0) {
		...
		if (new_outliers_cnt > 0) {
			if ((boxplot->outliers = (double*)malloc(new_outliers_cnt * sizeof(double))) == NULL) {
				throw PlotException(0, "Boxplot", "create( ... )", "outliers", new_outliers_cnt * sizeof(double));
			}
			...
			if ((boxplot->groups = (int*)malloc(boxplot->outliers_cnt * sizeof(int))) == NULL) {
				throw PlotException(0, "Boxplot", "create( ... )", "groups", boxplot->outliers_cnt * sizeof(int));
			}
	...
	/* Copy name */
	boxplot->names[0] = (char*)malloc((strlen(name) + 1) * sizeof(char));
	if (boxplot->names[0] == NULL) {
		throw PlotException(0, "Boxplot", "create( ... )", "name", name);
	}
	strcpy(boxplot->names[0], name);

	/* Set width */
	if ((boxplot->widths = (double*)malloc(sizeof(double))) == NULL) {
		throw PlotException(0, "Boxplot", "create( ... )", "widths", sizeof(double));
	}
...
Boxplot* Boxplot::merge(Boxplot* to, Boxplot* from) throw (PlotException) {
	...
	if ((from != NULL) && (from->get_groups_cnt() == 1)) {
		...
		if (new_memory == NULL) {
			throw PlotException(4, "Boxplot", "merge( ... )", "tukey", new_group * TUKEY_SIZE * sizeof(double));
		}
		...
		if (new_memory == NULL) {
			throw PlotException(4, "Boxplot", "merge( ... )", "observations", new_group * sizeof(int));
		}
		...
		if (new_memory == NULL) {
			throw PlotException(4, "Boxplot", "merge( ... )", "outliers", (boxplot->outliers_cnt + from->outliers_cnt) * sizeof(double));
		}
		boxplot->outliers = (double*)new_memory;

		new_memory = realloc(boxplot->groups, (boxplot->outliers_cnt + from->outliers_cnt) * sizeof(int));
		if (new_memory == NULL) {
			throw PlotException(4, "Boxplot", "merge( ... )", "groups", (boxplot->outliers_cnt + from->outliers_cnt) * sizeof(int));
		}
		...
		if (new_memory == NULL) {
			throw PlotException(4, "Boxplot", "merge( ... )", "widths", new_group * sizeof(double));
		}
		...
		if (new_memory == NULL) {
			throw PlotException(4, "Boxplot", "merge( ... )", "names", new_group * sizeof(char*));
		}
		...
		if (boxplot->names[boxplot->groups_cnt] == NULL) {
			throw PlotException(0, "Boxplot", "merge( ... )", "from->names[0]", from->names[0]);
		}
...
void Boxplot::set_name(int group, const char* name) throw (PlotException) {
	if (name == NULL) {
		throw PlotException(1, "Boxplot", "set_name( ... )", "name", "NULL");
	}

	if (strlen(name) <= 0) {
		throw PlotException(2, "Boxplot", "set_name( ... )", "name", (const char*)NULL);
	}

	if ((0 < group) && (group <= groups_cnt)) {
		group -= 1;
		free(names[group]);
		names[group] = NULL;
		names[group] = (char*)malloc((strlen(name) + 1) * sizeof(char));
		if (names[group] == NULL) {
			throw PlotException(0, "Boxplot", "set_name( ... )", "name", name);
		}

		strcpy(names[group], name);
	}
	else {
		throw PlotException(3, "Boxplot", "set_name( ... )", "group", group);
	}
}

void Boxplot::set_common_name(const char* name) throw (PlotException) {
	if (name == NULL) {
		throw PlotException(1, "Boxplot", "set_common_name( ... )", "name", "NULL");
	}

	if (strlen(name) <= 0) {
		throw PlotException(2, "Boxplot", "set_common_name( ... )", "name", (const char*)NULL);
	}
	...
	if (common_name == NULL) {
		throw PlotException(0, "Boxplot", "set_common_name( ... )", "name", name);
	}
...
void Boxplot::set_width(int group, double width) throw (PlotException) {
	if ((0 < group) && (group <= groups_cnt)) {
		...
		throw PlotException(3, "Boxplot", "set_width ( ... )", "group", group);
	}
}
...
void Boxplot::set_title(const char* title) throw (PlotException) {
	if (title == NULL) {
		throw PlotException(1, "Boxplot", "set_title( ... )", "title", "NULL");
	}

	if (strlen(title) <= 0) {
		throw PlotException(2, "Boxplot", "set_title( ... )", "title", (const char*)NULL);
	}
	...
	if (this->title == NULL) {
		throw PlotException(0, "Boxplot", "set_title( ... )", "title", title);
	}
