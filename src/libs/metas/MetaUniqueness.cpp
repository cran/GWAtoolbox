...
void MetaUniqueness::put(char* value) throw (MetaException) {
	...
	if (new_value == NULL) {
		throw MetaException(0, "MetaUniqueness", "put( ... )", "value", value);
	}
	...
	if (n > current_heap_size) {
		...
		if (new_data == NULL) {
			...
			free(new_value);
			data = NULL;
			throw MetaException(3, "MetaUniqueness", "put( ... )", "data", current_heap_size * sizeof(char*));
		}
		data = new_data;
	}
...
void MetaUniqueness::finalize() throw (MetaException) {
	if ((data != NULL) && (n > 0)) {
		...
		qsort(data, n, sizeof(char*), MetaUniqueness::compare);
		...
		for (int i = 0; i < n - 1; i++) {
			if (strcmp(data[i], data[i + 1]) == 0) {
				if ((prev_duplicate == NULL) || (strcmp(prev_duplicate, data[i + 1]) != 0)) {
					...
					if (new_string == NULL) {
						throw MetaException(0, "MetaUniqueness", "finalize( ... )", "data[i + 1]", data[i + 1]);
					}
...
void MetaUniqueness::print_html(ostream& stream, const char* image_name) {
	stream << resetiosflags(stream.flags());
...
int MetaUniqueness::compare (const void* first, const void* second) {
