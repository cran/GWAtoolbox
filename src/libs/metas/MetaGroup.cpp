...
MetaGroup::MetaGroup(): Meta(), na_value(false), new_value(NULL), groups(MetaGroup::compare) {
}
...
void MetaGroup::put(char* value) throw (MetaException) {
	...
	if (groups_it != groups.end()) {
		...
		if (new_value == NULL) {
			throw MetaException(0, "MetaGroup", "put( ... )", "value", value);
		}
...
void MetaGroup::print_html(ostream& stream, const char* image_name) {
	if (!groups.empty()) {
		map<int, int> numeric_groups;
		map<int, int>::iterator numeric_groups_it;
		vector< pair<const char*, int> > non_numeric_groups;
...
bool MetaGroup::compare (const char* first, const char* second) {
