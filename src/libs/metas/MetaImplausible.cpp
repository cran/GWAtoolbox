...
MetaImplausible::MetaImplausible(int heap_size)	throw (MetaException): MetaNumeric(heap_size), n_less(0), n_greater(0) {

}

MetaImplausible::MetaImplausible(double less, double greater, int heap_size) throw (MetaException): MetaNumeric(heap_size) {
	this->less = less;
	...
	if (numeric) {
		...
		if (n > current_heap_size) {
			...
			if (new_data == NULL) {
				free(data);
				data = NULL;
				throw MetaException(3, "MetaImplausible", "put( ... )", "data", current_heap_size * sizeof(double));
			}
			data = new_data;
		}
...
void MetaImplausible::print_html(ostream& stream, const char* prefix) {
	if (!numeric) {
		...
		stream << "<tr><td>&le; " << less << "</td><td>" << n_less << "</tr></td>";
		stream << "<tr><td>&ge; " << greater << "</td><td>" << n_greater << "</tr></td>";
		stream << "</table>";
		stream << "</div>";
